...
const state = {
  ...
  timeSpent: new Map(),
  // Firebase session support
  sessionId: null,
  testSession: null
};

// Convert Firebase question format to player format
function convertFirebaseQuestion(firebaseQuestion) {
  // Handle different question types
  const type = firebaseQuestion.type?.toUpperCase() || "MCQ";
  
  // Convert correct answers based on type
  let correctIndex = 0;
  if (type === "MCQ" || type === "TF") {
    correctIndex = Array.isArray(firebaseQuestion.correct) ? firebaseQuestion.correct[0] - 1 : (firebaseQuestion.correct || 0);
  } else if (type === "MSQ") {
    // MSQ: multiple correct answers
    correctIndex = firebaseQuestion.correct || [];
  } else if (type === "FIB") {
  ...
  return {
    id: firebaseQuestion.id,
    title: firebaseQuestion.title || "Untitled Question",
    prompt: firebaseQuestion.prompt || firebaseQuestion.description || "",
    type: type,
    difficulty: firebaseQuestion.difficulty || "easy",
    timeLimitSec: firebaseQuestion.timeLimit || firebaseQuestion.timeLimitSec || 60,
    options: firebaseQuestion.options || ["Option A", "Option B", "Option C", "Option D"],
    correctIndex: correctIndex,
    explanation: firebaseQuestion.explanation || "",
    tags: firebaseQuestion.tags || [],
    // Additional fields for different question types
    correct: firebaseQuestion.correct, // Keep original for validation
    marks: firebaseQuestion.marks || 2
  };
}

// Load test session from URL parameters
async function loadTestSession() {
  ...
  try {
    const sessionDoc = await api.getDoc(api.doc(db, 'test_sessions', sessionId));
    ...
    state.testSession = sessionData;
    state.examId = testId;
    
    // Convert Firebase questions to player format
    const firebaseQuestions = sessionData.questions || [];
    state.questions = firebaseQuestions.map(q => convertFirebaseQuestion(q));
    
    state.exam = {
      title: sessionData.testTitle,
      duration: sessionData.duration,
      totalMarks: sessionData.totalMarks
    };
    
    // Update UI
    el('examTitle').textContent = sessionData.testTitle || 'Test';
    el('examMeta').textContent = `${sessionData.questions?.length || 0} Questions • ${sessionData.duration} Minutes • ${sessionData.totalMarks} Marks`;
...
function mountQuestion() {
  ...
  setTimeout(() => {
    el("qHost").innerHTML = renderPlayer({ ...q, __qNo: state.idx + 1, __qTotal: state.questions.length });
...
async function confirmSubmit() {
  try {
    ...
    setTimeout(() => {
      window.location.href = new URL(`../results/results.html?exam=${encodeURIComponent(state.examId)}`, import.meta.url).href;
    }, 500);
  } catch (e) {
...
async function init() {
  try {
    ...
    if (sessionId) {
      // Load Firebase session
      const sessionLoaded = await loadTestSession();
      if (!sessionLoaded) {
        toast("Cannot load test session.", "error", 3200);
        return;
      }
      
      wire();
      
      // Initialize with Firebase session data
      const deviceId = getDeviceId();
    ...
    // Fallback to original exam-based flow
