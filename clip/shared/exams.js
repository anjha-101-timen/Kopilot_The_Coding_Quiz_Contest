  ...
  return `
    <article class="examCard" data-exam-id="${escapeHtml(exam.id)}">
      ...
      <div class="examActions">
        ...
        <a class="btn smallBtn" href="/results/results.html?exam=${encodeURIComponent(exam.id)}">Results</a>
        <a class="btn smallBtn ghost" href="/update/update.html?exam=${encodeURIComponent(exam.id)}">Manage</a>
      </div>
    </article>
  `;
}
