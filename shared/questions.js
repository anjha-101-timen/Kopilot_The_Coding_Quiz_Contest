...
export function renderQuestionCard(q, mode = "default") {
  ...
  const updateHref = new URL(`../update/update.html?id=${encodeURIComponent(q.id)}`, import.meta.url).href;
  const deleteHref = new URL(`../delete/delete.html?id=${encodeURIComponent(q.id)}`, import.meta.url).href;
  ...
  const actions =
    mode === "readonly"
      ...
      : `
        <div class="qActions">
          <button class="btn smallBtn" data-action="play">Play</button>
          <a class="btn smallBtn ghost" href="${updateHref}">Edit</a>
          <a class="btn smallBtn danger" href="${deleteHref}">Delete</a>
        </div>
      `;
...
export function renderPlayer(q) {
  const rawTitle = escapeHtml(q.title || "Untitled");
  const qNo = Number(q?.__qNo);
  const title = Number.isFinite(qNo) && qNo > 0 ? `${qNo}. ${rawTitle}` : rawTitle;
